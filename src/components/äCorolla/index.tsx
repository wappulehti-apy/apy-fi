...
class äCorolla extends React.Component {
  ...
  componentDidMount() {
    ...
    if (hasWebGL) {
      ...
      const diffuseMap = new THREE.TextureLoader().load(diffuse)
      const normalMap = new THREE.TextureLoader().load(normal)
      const roughnessMap = new THREE.TextureLoader().load(roughness)
      const alphaMap = new THREE.TextureLoader().load(opacity)
      const specularMap = new THREE.TextureLoader().load(specular)
      ...
      // Add ground
      const geometry = new THREE.PlaneBufferGeometry(128, 128, 128, 128)
      ...
      const DivNoWebGL = document.createElement('div')
      ...
      const äpyText = document.createTextNode('äpy')
